...
def tests_News_create_view(client):
    ...
    liked_user_list = test_helpers.create_User()
    ...
    data = {
        ...
        "liked_user_list": liked_user_list.pk,
    }
    response = client.post(url, data)
    assert response.status_code == 302
...
def tests_UserInfo_create_view(client):
    ...
    data = {
        ...
        "user_disliked_list": user_disliked_list.pk,
        "user_id": user_id.pk,
        "meetings_list": meetings_list.pk,
        "images_list": images_list.pk,
        "user_liked_list": user_liked_list.pk,
    }
    response = client.post(url, data)
    assert response.status_code == 302
